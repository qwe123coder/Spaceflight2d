...
vectorImages = {
    "player/1": pygame.image.load(os.path.join("data", "ship-1.png")),
...
Sounds = {
    "boom": pygame.mixer.Sound(os.path.join("data", "boom2.ogg")),
...
def Draw(update=True):
    ...
    if GamePaused and update:
        ...
        if playerShip.shoot > 0:
            pygame.draw.circle(
                Surface, (128, 128, 128), (midx, midy), int(200 / playerView.zoomfactor)
            )
            playerShip.shoot -= 1
        ...
        for Thing in PlanetContainer:
            ...
            if ExtendedVision:
                ...
                Surface.blit(
                    Font.render("Oil: " + str(int(Thing.oil)), True, (255, 255, 255)),
        ...
        pygame.draw.rect(
            Surface,
            (0, 255, 0),
            (40, 8, (SCR_SIZE[0] - 48) * playerShip.hull / playerShip.maxhull, 20),
            0,
        )
        ...
        Text = Font.render(
            "You are in Sector " + sectors.pixels2sector(playerShip.X, playerShip.Y),
            True,
            (255, 255, 255),
        )
...
def ChangeKey(keyname):
    ...
    while True:
        ...
        Surface.fill((0, 0, 0))
        Text = Font.render(
            "Press a new key for the action " + name + ".", True, (255, 255, 255)
        )
...
def SaveAs():
    ...
    while True:
        ...
        if (ticks // 8) % 2 == 0:
            ...
            pygame.draw.line(
                Surface, (0, 0, 0), (Left + 2 + X, Top + 2), (Left + 2 + X, Top + Y), 1
            )
        ...
        for file in FileList:
            if file.startswith(text):
                Surface.blit(Font.render(file, 1, (255, 255, 255)), (Left + 2, ypos))
                ypos += Y
        pygame.display.flip()
...
def Map():
    ...
    for system in SystemContainer:
        sysdraw.append(
            (
                int(
                    GRID_WIDTH * int((SCR_SIZE[0] / 2 + system[0]) / GRID_WIDTH)
                    + 0.5 * GRID_WIDTH
        ...
        for newPlanetX in range(-1, 2):
            for newPlanetY in range(-1, 2):
                ArchiveContainer.append(
                    Planet(
                        rndX + newPlanetX * 20000 + random.randint(-8000, 8000),
                        -rndY + newPlanetY * 18000 + random.randint(-6000, 6000),
                        random.randint(250, 1500),
                    )
                )
